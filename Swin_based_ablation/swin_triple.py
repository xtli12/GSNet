...
class NONLocalBlock2D(nn.Module):
    ...
    def forward(self, x):
        batch_size = x.size(0)
        g_x = self.g(x).view(batch_size, self.inter_channels, -1) # 1*1*c/r 
        g_x = g_x.permute(0, 2, 1) 
        theta_x = self.theta(x).view(batch_size, self.inter_channels, -1) 
        theta_x = theta_x.permute(0, 2, 1) 
        f = self.count_cov_second(theta_x)
        f_div_C = F.softmax(f, dim=-1) # softmax
...
class SwinTransformer(nn.Module):
    def __init__(self, *, hidden_dim, layers, heads, channels=3, num_classes=14, head_dim=32, window_size=7,
        ...
        self.cf = nn.Conv2d(2304, 768, kernel_size=1, stride=1, bias=False)
        ...
        self.ca = nn.Conv2d(768, 1536, kernel_size=1, stride=1, bias=False)
...
if __name__ == '__main__':
    batch_size = 8
    net = SwinTransformer(hidden_dim=96, layers=(2, 2, 6, 2), heads=(3, 6, 12, 24), window_size=7,num_classes=14)

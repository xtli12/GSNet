...
import shutil
# from swin_v2_32 import *
# from models_mae_32 import *
# from models_mae_224_10 import *
# from swin_v2_224 import *
# from swin_v2_tiny256 import *
# from swin1 import *
# from swin1 import *
# from GSNet_cifar_withtransition import *
# from swin_Den4KVStage3q_sota_10classes import *
...
# from Swin_S import *
# from Swin_S_32 import *
# from Swin_B import *
# from Swin_B_224 import *
...
parser.add_argument("--outf", type=str, default="./Results/ImageNet_convnext_xxlarge.clip_laion2b_soup_ft_in1k/", help='path log files') 
...
def main():
    ...
    # model = SwinTransformer(hidden_dim=96, layers=(2, 2, 6, 2), heads=(3, 6, 12, 24), growth_rate=32, block_config=(6, 12, 24, 16), window_size=11,
    #              num_init_features=64, bn_size=4, drop_rate=0)
...
'''
    ...
    if torch.cuda.is_available(): 
        model.cuda(); criterion_train.cuda();
    print('Model is already and parameters number is ', sum(param.numel() for param in model.parameters())) 

    start_epoch = 0; iteration = 0; record_acc = 0.5 # 训练代数、次数初始为0，损失初始为1000
    ...
    for epoch in range(start_epoch+1, opt.end_epoch):
        ...
        train_loss, iteration, lr = tra.train(data_loader, model, criterion_train, optimizer, epoch, iteration, opt.init_lr, opt.decay_power)
        ...
        print("Epoch [%02d], Iter[%06d], Time:%.9f, learning rate : %.9f, Train Loss: %.9f acc: %.9f " % (epoch, iteration, epoch_time, lr, train_loss, acc))
        record_loss(loss_csv,epoch, iteration, epoch_time, lr, train_loss, acc)
        logger.info("Epoch [%02d], Iter[%06d], Time:%.9f, learning rate: %.9f, Train Loss: %.9f acc: %.9f " % (epoch, iteration, epoch_time, lr, train_loss, acc)) 
...
class tra(nn.Module):
    ...
    def train(self, train_loader, model, criterion, optimizer, epoch, iteration, init_lr, decay_power): 
        ...
        for i, (images, labels) in enumerate(train_loader): 
            labels = labels.cuda(); labels = Variable(labels)
            images = images.cuda(); images = Variable(images)# 
            ...
            print('[Epoch:%02d],[Process:%d/%d],[iter:%d],lr=%.9f,train_losses.avg=%.9f' % (epoch, i, len(train_loader), iteration, lr, losses.avg)) 
